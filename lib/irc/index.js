...
Bot.prototype.init = function() {
	...
	var client = new Client(this.__profile);
	...
	client.on("part", this.listeners.part.bind(this));
...
Bot.prototype.listeners = {
	...
	part: function(channel, user) {
		this.emit("part", channel, user);

		Bot.prototype.log.call(this, this.LOG_JOIN, channel.client.name, "Parted", channel.name);
	},
	...
	message: function(channel, user, text) {
		console.log('message');
		var trimmed = text.trim();
		console.log(trimmed.match(/^\!([a-zA-Z0-9]){2,20}/g));
